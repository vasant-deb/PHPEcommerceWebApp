            ...
            success: function(data) {
                $.each(data.OrderItem, function(key, value) {
                    if($('#subtotal_' + key).html() != value.subtotal) {
                        ...
                        $('#subtotal_' + key).html('&pound' + value.subtotal).animate({ backgroundColor: "#ff8" }, 100).animate({ backgroundColor: "#fff" }, 500);
                    }
                });
                $('#subtotal').html('&pound' + data.Order.total).animate({ backgroundColor: "#ff8" }, 100).animate({ backgroundColor: "#fff" }, 500);
                $('#total').html('&pound' + data.Order.total).animate({ backgroundColor: "#ff8" }, 100).animate({ backgroundColor: "#fff" }, 500);
