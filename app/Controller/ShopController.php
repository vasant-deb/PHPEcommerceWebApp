...
//////////////////////////////////////////////////
    ...
    public function add() {
        ...
        if ($this->request->is('post')) {
			...
			$product_size  = @$this->request->data['Product']['product_size'];  
			$product_color = @$this->request->data['Product']['product_color'];
			$order_po      = @$this->request->data['Product']['order_po'];			 		
			 
            $product = $this->Cart->add($id, $quantity, $productmodId, $product_size, $product_color,$order_po);
        }
        if(!empty($product)) {
...
//////////////////////////////////////////////////
 ...
 public function itemupdate() {
        ...
        if ($this->request->is('ajax')) {
			...
			$product_size  = @$this->request->data['product_size'];  
			$product_color = @$this->request->data['product_color'];
...
public function generateRandomString($length = 10) {
 ...
 public function orderpayment() {
	 ...
	 $this->layout = "ajax";
	   ...
	   $order_id = 'mwoman_'.$user_id.$this->generateRandomString(6);
